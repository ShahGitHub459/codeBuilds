...
import Models.sallyBeautyObjects;
...
public class TestSally {
	...
	public void Test() throws IOException {
		for(int i=0;i<C3.rowLength;i++) {
			...
			if(sallyBeautyObjects.innerTextmarca(driver)!=null) {
				innerTextmarca=sallyBeautyObjects.innerTextmarca(driver).getText();
			}
			else {
				innerTextmarca=sallyBeautyObjects.innerTextmarca(driver).getText();
			}
			if(sallyBeautyObjects.innerTextnombre(driver)!=null) {
				innerTextnombre=sallyBeautyObjects.innerTextnombre(driver).getText();
			}
			else {
				innerTextnombre="Not Found from element";
			}
			if(sallyBeautyObjects.innerTextprice(driver)!=null) {
				innerTextprice=sallyBeautyObjects.innerTextprice(driver).getText();
			}
			else {
				innerTextprice="Not Found from element";
			}
			if(sallyBeautyObjects.innerTextean(driver)!=null) {
				innerTextean=sallyBeautyObjects.innerTextean(driver).getText();
			}
			else {
				innerTextean="Not Found from element";
			}
			if(sallyBeautyObjects.availabilityCheck(driver)!=null) {
				Availabilityflag=sallyBeautyObjects.availabilityCheck(driver);
			}
			else {
			...
			if(!C3.precio[i].toString().equals("N")) {
				boolean areTheyEqual = new BigDecimal(C3.precio[i].toString()).compareTo(new BigDecimal(innerTextprice.replaceAll("[$]", ""))) == 0;
